...
async function getHomeStats() {
  ...
  const [
    ...
    supabase.from('niche_analytics').select('views_total').order('polled_at', { ascending: false }).limit(10),
  ])

  const totalPending = (pendingGate1 ?? 0) + (pendingGate2 ?? 0) + (pendingGate3 ?? 0) + (pendingGate5 ?? 0) + (pendingGate6 ?? 0)
  const weeklyViews = (recentAnalytics ?? []).reduce((sum, r) => sum + r.views_total, 0)
